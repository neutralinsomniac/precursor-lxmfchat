...
impl ScrollableList {
    ...
    pub fn move_selection(&mut self, dir: Direction) {
        self.select_index = match dir {
            // Up/Down wrap around within the current column, so a long list can be
            // navigated past either end (e.g. reaching a top "cancel" item quickly).
            Direction::Up => {
                let col = self.select_index.0;
                let len = self.items[col].len();
                let row = if self.select_index.1 == 0 { len.saturating_sub(1) } else { self.select_index.1 - 1 };
                (col, row)
            }
            Direction::Down => {
                let col = self.select_index.0;
                let len = self.items[col].len().max(1);
                (col, (self.select_index.1 + 1) % len)
            }
