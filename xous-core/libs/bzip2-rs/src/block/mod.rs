...
impl Block {
    pub fn new(header: Header) -> Self {
        Self {
            ...
            // PRECURSOR PATCH: was `Vec::with_capacity(header.max_blocksize())`
            // — 3.6 MB up front for a "BZh9" stream, an OOM abort on the
            // device. `tt` only ever grows by `push`/`resize` up to the actual
            // block content (bounded by the clamped `max_blocksize`), so let
            // it grow on demand instead of preallocating the worst case.
            tt: Vec::new(),
            pre_rle_used: 0,
            t_pos: 0,
            last_byte: -1,
