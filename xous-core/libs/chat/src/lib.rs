        ...
        match FromPrimitive::from_usize(msg.body.id()) {
            ...
            Some(ChatOp::PostDel) => {
                xous::msg_scalar_unpack!(msg, index, _, _, _, {
                    ...
                    if let Err(e) = ui.post_del(index) {
                        log::warn!("ChatOp::PostDel {index} failed: {e:?}");
                    }
                });
            }
            ...
            Some(ChatOp::PostUpdate) => {
                let mut buffer = unsafe { Buffer::from_memory_message(msg.body.memory_message().unwrap()) };
                if let Ok(mut pu) = buffer.to_original::<api::PostUpdate, _>() {
                    pu.found = ui.post_update(pu.author.as_str(), pu.timestamp, pu.text.as_str());
                    if pu.found {
                        ui.redraw().ok();
                    }
                    buffer.replace(pu).expect("couldn't serialize return");
                } else {
                    log::warn!("failed to deserialize PostUpdate");
                }
            }
...
/// thread). Find+replace happen in one server op, so it is safe against
/// concurrent posts shifting indices. Persist afterwards with `DialogueSave`.
pub fn cf_post_update(chat_cid: xous::CID, author: &str, timestamp: u64, text: &str) -> bool {
    let pu = PostUpdate { author: author.to_string(), timestamp, text: text.to_string(), found: false };
    let mut buf = match Buffer::into_buf(pu) {
        Ok(b) => b,
        Err(_) => return false,
    };
    if buf.lend_mut(chat_cid, ChatOp::PostUpdate as u32).is_err() {
        return false;
    }
...
/// background network thread) that only hold a connection id to the chat server.
pub fn cf_set_status_idle_text(chat_cid: xous::CID, msg: &str) {
    let bm = BusyMessage { busy_msg: String::from(msg) };
    Buffer::into_buf(bm)
        .expect("internal error")
        .send(chat_cid, ChatOp::SetStatusIdleText as u32)
        .expect("internal error");
}
